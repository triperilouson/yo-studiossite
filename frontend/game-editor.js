...
const state = {
    ...
    levels: [], level: { slug: "yo-showroom", name: "YO SHOWROOM", width: 1280, height: 768, isActive: true, layers: defaultLayers.map((layer) => ({ ...layer })), objects: [] },
...
function layerFor(id) {
    return state.level.layers.find((layer) => layer.id === id) || { id, name: id || "OBJECTS", order: 0, visible: true, locked: false };
}

function visibleLevelObjects() {
    return state.level.objects.filter((object) => layerFor(object.layer).visible !== false);
}
...
function drawLevel() {
    ...
    const sorted=[...visibleLevelObjects()].sort((a,b)=>(layerFor(a.layer).order-layerFor(b.layer).order)||((a.y+a.depthOffset)-(b.y+b.depthOffset)));
...
function renderObjectLayerOptions(){const select=document.getElementById("object-layer");if(!select)return;const current=state.selectedObject?.layer;select.replaceChildren(...state.level.layers.sort((a,b)=>a.order-b.order).map(layer=>new Option(layer.name,layer.id)));if(current)select.value=current;}
function renderLayerList(){const list=document.getElementById("level-layers");if(!list)return;list.replaceChildren();state.level.layers.sort((a,b)=>a.order-b.order).forEach(layer=>{const row=document.createElement("div");row.className="layer-row";const visible=document.createElement("button");visible.type="button";visible.textContent=layer.visible===false?"○":"●";visible.title="Toggle visibility";visible.addEventListener("click",()=>{layer.visible=layer.visible===false;renderLayerList();});const locked=document.createElement("button");locked.type="button";locked.textContent=layer.locked?"L":"U";locked.title="Toggle lock";locked.addEventListener("click",()=>{layer.locked=!layer.locked;renderLayerList();});const name=document.createElement("input");name.dataset.layerField="name";name.value=layer.name;name.addEventListener("input",()=>{layer.name=name.value.toUpperCase();renderObjectLayerOptions();});const order=document.createElement("input");order.type="number";order.dataset.layerField="order";order.value=layer.order;order.addEventListener("input",()=>{layer.order=Number(order.value);renderLayerList();});const remove=document.createElement("button");remove.type="button";remove.textContent="×";remove.title="Remove empty layer";remove.disabled=state.level.objects.some(object=>object.layer===layer.id);remove.addEventListener("click",()=>{state.level.layers=state.level.layers.filter(item=>item!==layer);renderLayerList();renderObjectLayerOptions();});row.append(visible,locked,name,order,remove);list.append(row);});renderObjectLayerOptions();}
...
function objectAt(point){return[...visibleLevelObjects()].reverse().find(object=>{const asset=state.assets.find(item=>item.id===object.assetId);if(!asset)return false;const size=objectSize(object,asset);const width=size.width*object.scaleX,height=size.height*object.scaleY;return point.x>=object.x-width/2&&point.x<=object.x+width/2&&point.y>=object.y-height&&point.y<=object.y;});}
function selectObject(object){state.selectedObject=object;const fields=document.getElementById("object-fields");fields.classList.toggle("disabled",!object);renderObjectLayerOptions();if(!object)return;fields.querySelectorAll("[data-object]").forEach(input=>{const key=input.dataset.object;input[input.type==="checkbox"?"checked":"value"]=object[key];});}
function addToLevel(){if(!state.selected){setStatus("SELECT AN ASSET",true);return;}const layer=state.level.layers.find(item=>item.id==="furniture")||state.level.layers[0];if(layer?.locked){setStatus("SELECTED LAYER IS LOCKED",true);return;}const object={id:crypto.randomUUID(),assetId:state.selected.id,x:snap(state.level.width/2),y:snap(state.level.height/2),z:0,rotation:0,scaleX:1,scaleY:1,flipX:false,flipY:false,layer:layer?.id||"furniture",depthOffset:0,locked:false};state.level.objects.push(object);selectObject(object);switchMode("level");}
function duplicateObject(){if(!state.selectedObject||layerFor(state.selectedObject.layer).locked)return;const copy={...clone(state.selectedObject),id:crypto.randomUUID(),x:state.selectedObject.x+16,y:state.selectedObject.y+16,locked:false};state.level.objects.push(copy);selectObject(copy);}
function removeObject(){if(!state.selectedObject||state.selectedObject.locked||layerFor(state.selectedObject.layer).locked)return;state.level.objects=state.level.objects.filter(object=>object!==state.selectedObject);selectObject(null);renderLayerList();}
...
function handleLevelPointerDown(event){const point=levelPoint(event);if(event.button===1||event.buttons===4){const raw=canvasPoint(event);state.panDrag={point:raw,startX:state.panX,startY:state.panY};return;}let object=objectAt(point);const layerLocked=object?layerFor(object.layer).locked:false;if(event.altKey&&object&&!layerLocked){object={...clone(object),id:crypto.randomUUID(),x:object.x+8,y:object.y+8,locked:false};state.level.objects.push(object);renderLayerList();}selectObject(object||null);if(object&&!object.locked&&!layerLocked)state.objectDrag={object,offsetX:point.x-object.x,offsetY:point.y-object.y,startX:object.x,startY:object.y};}
...
document.getElementById("add-layer").addEventListener("click",()=>{const name=window.prompt("Layer name","FOREGROUND");if(!name)return;const id=name.trim().toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"layer";if(state.level.layers.some(layer=>layer.id===id)){setStatus("LAYER ALREADY EXISTS",true);return;}state.level.layers.push({id,name:name.trim().toUpperCase(),order:Math.max(0,...state.level.layers.map(layer=>layer.order))+50,visible:true,locked:false});renderLayerList();});
document.getElementById("reset-layers").addEventListener("click",()=>{state.level.layers=clone(defaultLayers);state.level.objects.forEach(object=>{if(!state.level.layers.some(layer=>layer.id===object.layer))object.layer="furniture";});renderLayerList();selectObject(state.selectedObject);});
document.querySelectorAll("[data-object]").forEach(input=>input.addEventListener("input",()=>{if(!state.selectedObject)return;const key=input.dataset.object;state.selectedObject[key]=input.type==="checkbox"?input.checked:(input.type==="number"?Number(input.value):input.value);if(key==="layer")renderLayerList();}));
...
document.getElementById("import-level").addEventListener("click",()=>{try{const data=JSON.parse(document.getElementById("level-json").value);applyLevel({...data,config:{layers:data.layers||data.config?.layers||defaultLayers,objects:data.objects||data.config?.objects||[]}});setStatus("LEVEL JSON LOADED");}catch{setStatus("INVALID LEVEL JSON",true);}});
